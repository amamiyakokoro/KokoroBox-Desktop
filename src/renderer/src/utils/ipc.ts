...
export async function scanAppRoutingDirectory(
  ...
  return ipcErrorWrapper(
    await window.electron.ipcRenderer.invoke('scanAppRoutingDirectory', directoryPath)
  )
}
...
export async function openServiceSystemSettings(): Promise<void> {
  return ipcErrorWrapper(await window.electron.ipcRenderer.invoke('openServiceSystemSettings'))
}
