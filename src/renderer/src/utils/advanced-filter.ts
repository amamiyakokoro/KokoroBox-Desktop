...
class FilterParser {
  ...
  parse(): FilterExpression {
    ...
    if (this.current().type !== 'eof') {
      throw new FilterSyntaxError(tr('Unexpected trailing content'), this.current().position)
    }
  ...
  private parsePrimary(): FilterExpression {
    ...
    if (token.type === 'lparen') {
      this.index += 1
      const expression = this.parseOr()
      this.expect('rparen', tr('Missing closing parenthesis ")"'))
      return expression
    }
    ...
    throw new FilterSyntaxError(tr('Missing valid expression'), token.position)
  }

  private parsePath(): FilterExpression {
    const segments: Array<string | number> = []
    this.expect('dot', tr('Path must start with "."'))
      ...
      if (this.current().type === 'identifier') {
        ...
        if (token.type === 'number') {
          ...
          throw new FilterSyntaxError(
            tr('Brackets support only numeric indices or string keys'),
            token.position
          )
        }

        this.expect('rbracket', tr('Missing closing bracket "]"'))
        expectSegment = false
      } else if (expectSegment) {
        throw new FilterSyntaxError(tr('Missing field name after dot'), this.current().position)
      } else {
        break
      }
  ...
  private consume<T extends FilterToken['type']>(type: T): Extract<FilterToken, { type: T }> {
    return this.expect(type, tr('Missing {0}', [type]))
  }
}
...
function parseQuotedString(input: string, start: number): { value: string; nextIndex: number } {
  ...
  while (index < input.length) {
    ...
    if (char === '\\') {
      ...
      if (!escaped) {
        throw new FilterSyntaxError(tr('Incomplete string escape'), index)
      }
  ...
  throw new FilterSyntaxError(tr('Missing closing quote in string'), start)
}
...
function tokenize(input: string): FilterToken[] {
  ...
  while (index < len) {
    ...
    throw new FilterSyntaxError(tr('Unrecognized character "{0}"', [input[index]]), index)
  }
...
function formatFilterError(error: unknown): string {
  if (error instanceof FilterSyntaxError) {
    return tr('Advanced filter syntax error: {0} (position {1})', [
      error.message,
      error.position + 1
    ])
  }

  return tr('Failed to parse advanced filter')
}
