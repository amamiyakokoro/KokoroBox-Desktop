...
export const isIPv4 = (ip: string): ValidationResult => {
  if (!ip) return { ok: false, error: tr('IP 地址不能为空') }
  try {
    return isIp.isIPv4(ip) ? { ok: true } : { ok: false, error: tr('无效的 IPv4 地址') }
  } catch (e) {
    return { ok: false, error: tr('解析 IP 地址时出错') }
  }
}
...
export const isIPv6 = (ip: string): ValidationResult => {
  if (!ip) return { ok: false, error: tr('IP 地址不能为空') }
  try {
    return isIp.isIPv6(ip) ? { ok: true } : { ok: false, error: tr('无效的 IPv6 地址') }
  } catch (e) {
    return { ok: false, error: tr('解析 IP 地址时出错') }
  }
}
...
export const isValidIPv4Cidr = (s: string | undefined): ValidationResult => {
  ...
  try {
    ...
    if (r === 6) return { ok: false, error: tr('这是 IPv6 CIDR，而不是 IPv4 CIDR') }
    return { ok: false, error: tr('不是有效的 CIDR 格式（示例：198.18.0.1/16）') }
  } catch (e) {
    return { ok: false, error: tr('解析 CIDR 时出错') }
  }
}
...
export const isValidIPv6Cidr = (s: string | undefined): ValidationResult => {
  ...
  try {
    ...
    if (r === 4) return { ok: false, error: tr('这是 IPv4 CIDR，而不是 IPv6 CIDR') }
    return { ok: false, error: tr('不是有效的 CIDR 格式（示例：fc00::/18）') }
  } catch (e) {
    return { ok: false, error: tr('解析 CIDR 时出错') }
  }
}
...
export const isValidPort = (s: string): ValidationResult => {
  if (!/^\d+$/.test(s)) return { ok: false, error: tr('端口应为数字') }
  const p = Number(s)
  return p >= 1 && p <= 65535 ? { ok: true } : { ok: false, error: tr('端口应在 1 到 65535 之间') }
}
...
export const isValidListenAddress = (s: string | undefined): ValidationResult => {
  ...
  if (idx === -1) return { ok: false, error: tr('应包含端口号') }
  const host = v.slice(0, idx)
  const port = v.slice(idx + 1)
  if (!isValidPort(port)) return { ok: false, error: tr('端口号不合法') }
...
export const isValidDomainWildcard = (s: string | undefined): ValidationResult => {
  ...
  if (v.startsWith('+.')) {
    ...
    return { ok: false, error: tr('+. 开头后应为合法的域名，例如 +.lan') }
  }

  if (v.startsWith('.')) {
    const domain = v.slice(1)
    if (/^[a-zA-Z0-9-]+(\.[a-zA-Z0-9-]+)*$/.test(domain)) return { ok: true }
    return { ok: false, error: tr('. 开头后应为合法的域名') }
  }

  if (v.includes('*')) {
    const labels = v.split('.')
    if (labels.every((lab) => lab === '*' || /^[a-zA-Z0-9-]+$/.test(lab))) return { ok: true }
    return { ok: false, error: tr('通配符位置或标签不合法') }
  }

  if (/^[a-zA-Z0-9-]+(\.[a-zA-Z0-9-]+)*$/.test(v)) return { ok: true }
  return { ok: false, error: tr('不是合法的域名或通配符表达式') }
}
...
export const isValidDnsServer = (s: string | undefined, ipOnly = false): ValidationResult => {
  ...
  if (!serverPart) return { ok: false, error: tr('服务器地址不能为空') }
  if (hashIndex !== -1) {
    ...
    for (const param of params) {
      if (param.includes('=')) {
        ...
        if (!key || !value) {
          return {
            ok: false,
            error: tr('参数格式不合法，key=value 格式中 key 和 value 都不能为空')
          }
        }
        ...
        if (!param || param.trim() === '') {
          return { ok: false, error: tr('参数不能为空') }
        }
        if (!/^[a-zA-Z0-9\u4e00-\u9fa5\s-_]+$/.test(param)) {
          return {
            ok: false,
            error: tr('参数 "{0}" 不合法，如有需要请关闭软件 DNS 覆盖使用', [param])
          }
        }
      }
    }
  ...
  if (lower.startsWith('dhcp://')) {
    ...
    if (!rest) return { ok: false, error: tr('dhcp:// 后应跟接口名或 system') }
    if (rest.toLowerCase() === 'system') return { ok: true }
    if (/^[a-zA-Z0-9_.-]+$/.test(rest)) return { ok: true }
    return { ok: false, error: tr('dhcp 接口名只允许字母数字、下划线、点或连字符') }
  }
  ...
  if (lower.startsWith('rcode://')) {
    ...
    return allowed.has(code) ? { ok: true } : { ok: false, error: tr('无效的 rcode 值') }
  }

  if (/^https?:\/\//i.test(serverPart)) {
    try {
      const u = new URL(serverPart)
      if (!u.hostname) return { ok: false, error: tr('无效的 URL 主机名') }
      if (ipOnly) {
        const hostname = u.hostname
        if (/^[0-9.]+$/.test(hostname)) {
          const r = isIPv4(hostname)
          if (!r.ok) return { ok: false, error: tr('无效的 IPv4 地址') }
          return { ok: true }
        }
        if (hostname.includes(':')) {
          const r = isIPv6(hostname)
          if (!r.ok) return { ok: false, error: tr('无效的 IPv6 地址') }
          return { ok: true }
        }
        return { ok: false, error: tr('主机必须是 IP 地址，不支持域名') }
      }
      return { ok: true }
    } catch (e) {
      return { ok: false, error: tr('无效的 URL 格式') }
    }
  }
  ...
  if (schemeMatch) {
    ...
    if (!['udp', 'tcp', 'tls', 'quic'].includes(scheme)) {
      return { ok: false, error: tr('不支持的协议：') + scheme }
    }
    ...
    if (!host) return { ok: false, error: tr('{0} 地址缺少主机', [scheme]) }
    if (/^[0-9.]+$/.test(host)) {
      const r = isIPv4(host)
      if (!r.ok) return { ok: false, error: tr('无效的 IPv4 地址') }
      ...
      if (!/^[a-zA-Z0-9-]+(\.[a-zA-Z0-9-]+)*$/.test(host)) {
        return { ok: false, error: tr('无效的主机名') }
      }
    }
    if (portStr) {
      if (!/^[0-9]+$/.test(portStr)) return { ok: false, error: tr('端口格式不正确') }
      const p = Number(portStr)
      if (p < 1 || p > 65535) return { ok: false, error: tr('端口超出范围') }
    }
    return { ok: true }
  }
  ...
  if (idx !== -1 && serverPart.includes(']') === false) {
    ...
    if (!/^[0-9]+$/.test(port)) return { ok: false, error: tr('端口格式不正确') }
    if (!host) return { ok: false, error: tr('主机不能为空') }
    if (/^[0-9.]+$/.test(host)) {
      const r = isIPv4(host)
      return r.ok ? { ok: true } : { ok: false, error: tr('无效的 IPv4 地址') }
    }
    if (host.startsWith('[') && host.endsWith(']')) {
      const inner = host.slice(1, -1)
      const r = isIPv6(inner)
      return r.ok ? { ok: true } : { ok: false, error: tr('无效的 IPv6 地址') }
    }
    if (ipOnly) {
      return { ok: false, error: tr('主机必须是 IP 地址，不支持域名') }
    }
  ...
  return { ok: false, error: tr('无效的服务器地址') }
}
