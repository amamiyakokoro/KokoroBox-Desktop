...
test('Kokoro subscription resolve and config download send the same platform User-Agent', async () => {
  ...
  assert.ok(h.requests.every((request) => request.headers['User-Agent'] === expectedUserAgent))
...
test('packaging registers the shared scheme and Windows callback relay precedes app locking', () => {
  ...
  const initialDeepLinks = main.indexOf('takeInitialDeepLinks(process.argv)')
...
test('Windows packaging runs as the current user and never self-elevates at startup', () => {
  ...
  assert.equal(existsSync('build/windows/runner/main.go'), false)
...
test('Windows privilege changes use explicit non-persistent native relaunches', () => {
  ...
  assert.match(
    startup,
    /mainWindow\.on\('close',[\s\S]*?if \(isAppQuitting\(\)\) return[\s\S]*?event\.preventDefault\(\)/
  )
  assert.match(
    startup,
