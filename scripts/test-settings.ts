...
test('desktop sidebar separates controls, live status and navigation', () => {
  ...
  assert.match(statusIndicator, /inline-flex min-w-0 gap-2/)
  assert.match(statusIndicator, /allowTextWrap \? 'break-words' : 'truncate'/)
  ...
  assert.match(appRouting, /<SiderStatusRow allowTextWrap/)
  assert.match(appRouting, /tone=\{statusTone\}/)
  assert.match(appRouting, /metadataCanWrap/)
  ...
  assert.match(surfaces, /metadata\s*\? cn\(/)
  assert.match(surfaces, /metadataCanWrap \? 'grid-rows-\[1\.25rem_auto\]'/)
