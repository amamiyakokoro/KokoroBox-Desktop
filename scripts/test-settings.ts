...
test('desktop sidebar separates controls, live status and navigation', () => {
  ...
  assert.match(rules, /endMetadata=\{rules\?\.rules\?\.length \?\? 0\}/)
  assert.doesNotMatch(rules, /description=\{String\(rules\?\.rules\?\.length/)
  ...
  assert.match(surfaces, /endMetadata\?: React\.ReactNode/)
  assert.match(navItem, /endMetadata !== undefined && endMetadata !== null/)
  assert.match(navItem, /grid-cols-\[2rem_minmax\(0,1fr\)_auto_2rem\]/)
  assert.match(navItem, /data-sider-end-metadata/)
  assert.match(navItem, /shrink-0 text-xs tabular-nums text-muted/)
  assert.match(navItem, /\{endMetadata\}[\s\S]*<LuChevronRight/)
  ...
  assert.doesNotMatch(overrideNav, /endMetadata=/)
  assert.doesNotMatch(logNav, /endMetadata=/)
