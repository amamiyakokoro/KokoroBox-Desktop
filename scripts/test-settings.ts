...
test('SettingItem has one canonical layout without legacy compatibility paths', () => {
  ...
  assert.match(settingSubgroup, /<div/)
  assert.doesNotMatch(settingSubgroup, /<Surface|variant="secondary"|bg-surface-secondary/)
...
test('conditional settings use the shared dependent subgroup hierarchy', () => {
  ...
  assert.equal(systemProxy.match(/<SettingSubgroup/g)?.length, 2)
...
test('desktop sidebar separates controls, live status and navigation', () => {
  ...
  const outboundModeModel = readFileSync(
    'src/renderer/src/components/sider/outbound-mode.ts',
    'utf8'
  )
  ...
  const overrideNav = readFileSync('src/renderer/src/components/sider/override-card.tsx', 'utf8')
  ...
  assert.match(sider, /SiderSection title="Kokoro"/)
  assert.match(sider, /SiderSection title=\{tr\('Current status'\)\}/)
  assert.match(sider, /SiderSection title=\{tr\('Navigation'\)\}/)
  const accountEntryPosition = sider.indexOf('SiderSection title="Kokoro"')
  ...
  assert.equal(quickControl.match(/<ToggleButton/g)?.length, 1)
  assert.doesNotMatch(quickControl, /<button/)
  ...
  assert.match(
    quickControlStyles,
    /\.sider-quick-control\s*\{[\s\S]*grid-template-columns:\s*minmax\(0, 1fr\)/
  )
  assert.match(
    quickControlStyles,
    /\.sider-quick-control__title\s*\{[\s\S]*grid-column:\s*1;[\s\S]*grid-row:\s*2/
  )
  assert.match(quickControlStyles, /\.sider-quick-control__status\s*\{/)
  assert.doesNotMatch(quickControlStyles, /sider-quick-control__(?:primary|control)/)
  assert.match(quickControlStyles, /grid-template-rows:\s*2rem 1\.25rem 1rem/)
  assert.match(quickControlStyles, /min-height:\s*5\.5rem/)
  assert.match(quickControlStyles, /@container \(min-width:\s*10rem\)/)
  assert.match(
    quickControlStyles,
    /@container[\s\S]*grid-template-columns:\s*2rem minmax\(0, 1fr\)/
  )
  ...
  assert.match(tun, /<SiderIconToggleButton/)
  assert.match(tun, /status=\{enable \? tr\('Enabled'\) : tr\('Disabled'\)\}/)
  assert.match(tun, /onToggle=\{onChange\}/)
  assert.match(tun, /isSelected=\{Boolean\(enable\)\}/)
  assert.match(tun, /isDragging=\{isDragging\}/)
  assert.match(tun, /onChange=\{onChange\}/)
  assert.doesNotMatch(tun, /<Switch|control=\{|<SiderIconButton/)
  ...
  assert.match(appRouting, /<SiderStatusRow[\s\S]*overflow-visible[\s\S]*text-clip/)
  assert.match(appRouting, /tone=\{statusTone\}/)
  assert.match(appRouting, /flex-nowrap items-center gap-1\.5 whitespace-nowrap/)
  ...
  assert.match(proxy, /useControledMihomoConfig\(\)/)
  assert.match(proxy, /const \{ mode = 'rule' \} = controledMihomoConfig \|\| \{\}/)
  assert.match(proxy, /group\.name\.toUpperCase\(\) === 'GLOBAL'/)
  assert.doesNotMatch(proxy, /\?\? groups\[0\]/)
  assert.match(proxy, /description=\{modeLabel\}/)
  assert.match(proxy, /mode === 'global'[\s\S]*globalGroup\?\.now/)
  assert.match(proxy, /mode === 'rule'[\s\S]*tr\('\{0\} groups'/)
  assert.match(proxy, /metadataSeparator=\{mode === 'global' \? '→' : '·'\}/)
  assert.match(proxy, /const cardLabel = `\$\{tr\('Proxy'\)\} — \$\{modeLabel\}`/)
  assert.match(proxy, /<SiderIconButton[\s\S]*label=\{cardLabel\}/)
  assert.match(outboundModeModel, /case 'rule':[\s\S]*tr\('Rules'\)/)
  assert.match(outboundModeModel, /case 'global':[\s\S]*tr\('Global'\)/)
  assert.match(outboundModeModel, /case 'direct':[\s\S]*tr\('Direct'\)/)
  assert.match(outboundMode, /getOutboundModeLabel\('rule'\)/)
  assert.match(outboundMode, /getOutboundModeLabel\('global'\)/)
  assert.match(outboundMode, /getOutboundModeLabel\('direct'\)/)
  ...
  const corePrimary = core.slice(core.indexOf('<SiderStatusCard'), core.indexOf('actions={'))
  assert.match(core, /<SiderIconDisplay label=\{tr\('Core'\)\} placement="right">/)
  assert.doesNotMatch(core, /settingsPath|useNavigate|useLocation|useSearchParams/)
  assert.doesNotMatch(core, /onPress=\{\(\) => navigate/)
  assert.doesNotMatch(corePrimary, /active=|onPress=/)
  ...
  assert.match(dns, /onToggle=\{onChange\}/)
  assert.match(dns, /isSelected=\{controlDns\}/)
  assert.match(dns, /isDragging=\{isDragging\}/)
  assert.match(dns, /onChange=\{onChange\}/)
  assert.doesNotMatch(dns, /<Switch|control=\{|<SiderIconButton/)
  assert.match(dns, /patchAppConfigOrThrow\(\{ controlDns: value \}\)/)
  assert.match(dns, /patchControledMihomoConfigOrThrow\(\{\}\)/)
  ...
  assert.match(sniff, /<SiderIconToggleButton/)
  assert.match(sniff, /\sstatus=\{controlSniff \? tr\('Enabled'\)/)
  assert.match(sniff, /enabled=\{controlSniff\}/)
  assert.match(sniff, /onToggle=\{onChange\}/)
  assert.match(sniff, /isSelected=\{controlSniff\}/)
  assert.match(sniff, /isDragging=\{isDragging\}/)
  assert.match(sniff, /onChange=\{onChange\}/)
  assert.doesNotMatch(sniff, /<Switch|control=\{|<SiderIconButton/)
  assert.match(sniff, /patchAppConfigOrThrow\(\{ controlSniff: value \}\)/)
  assert.match(sniff, /patchControledMihomoConfigOrThrow\(\{\}\)/)
  ...
  assert.match(rules, /description=\{tr\('\{0\} rules', \[rules\?\.rules\?\.length \?\? 0\]\)\}/)
  ...
  assert.match(appRouting, /metadata=\{/)
  assert.doesNotMatch(appRouting, /description=\{|status=\{runtimeLabel\}/)
...
test('proxy group rows stay compact while preserving semantic metadata and actions', () => {
  ...
  assert.match(header, /bg-surface hover:border-accent\/25/)
  assert.doesNotMatch(header, /bg-background/)
  assert.doesNotMatch(header, /bg-surface\/65/)
  assert.doesNotMatch(page, /searchVisible|searchValue|onUpdateSearch|CollapseInput/)
  assert.doesNotMatch(page, /Proxy group actions/)
  assert.match(page, /<KokoToolbar[\s\S]*?aria-label=\{tr\('Proxy groups'\)\}/)
  assert.match(page, /contentClassName="bg-surface"/)
  assert.match(page, /className="shrink-0 border-b border-separator bg-surface"/)
  assert.match(page, /ref=\{proxyListRef\} className="min-h-0 min-w-0 flex-1 bg-surface"/)
  assert.match(page, /<KokoSearchField/)
  assert.match(page, /tr\('Search proxy groups'\)/)
  assert.match(page, /const \[filter, setFilter\] = useState\(''\)/)
  assert.match(page, /const visibleGroupIndices = useMemo/)
  assert.match(page, /\[group\.name, group\.type, group\.now\]/)
  assert.match(page, /groupCounts=\{visibleGroupCounts\}/)
  assert.match(page, /visibleGroupIndicesRef\.current\[visibleIndex\]/)
  assert.match(page, /visibleGroupCountsRef\.current/)
  assert.match(page, /tr\('No proxy groups match this search\.'\)/)
  ...
  assert.match(item, /border-separator\/60 bg-surface hover:border-accent\/25/)
  assert.doesNotMatch(item, /border-separator\/60 bg-surface-secondary\/40/)
