...
function plistString(infoPath, key) {
  try {
    return execFileSync('/usr/bin/plutil', ['-extract', key, 'raw', infoPath], {
      encoding: 'utf8'
    }).trim()
  } catch {
    return ''
...
module.exports = async function afterPack(context) {
  ...
  const extensionInfo = path.join(extensionPath, 'Contents', 'Info.plist')
  const extensionIdentifier = plistString(extensionInfo, 'CFBundleIdentifier')
  ...
  if (
    !plistString(appInfo, 'NSSystemExtensionUsageDescription') ||
    !plistString(extensionInfo, 'NSSystemExtensionUsageDescription')
  ) {
    throw new Error('macOS host and System Extension must include usage descriptions')
  }
