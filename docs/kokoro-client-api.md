...
- The Electron single-instance lock is acquired **before Windows elevation or startup side effects**. A secondary launch hands its arguments to the original app instance; that instance owns the verifier. It does not launch another application to handle a Kokoro callback.
